...
def main():
    parser = argparse.ArgumentParser(description='Polish nesting from Inkscape SVGs.')
    ...
    all_shelves = load_shelves(project_dir)
    shelf_dict  = {s.name: s for s in all_shelves}
    print(f"  Loaded {len(shelf_dict)} shelves.")

    svg_files = sorted((project_dir / 'Inkscape').glob('*.svg'))
